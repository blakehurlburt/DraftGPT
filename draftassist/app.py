            ...
            recs[rp] = [
                {
                    ...
                    "projected_games": round(r.player.projected_games) if r.player.projected_games > 0 else None,
    ...
    for p in sorted(state.available, key=lambda p: p.projected_total, reverse=True):
        if p.is_rookie:
            available_rookies.append({
                ...
                "projected_games": round(p.projected_games) if p.projected_games > 0 else None,
                "bye_week": p.bye_week,
                "is_rookie": True,
